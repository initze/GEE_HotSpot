...
def tcS2(image):
    ...
    """
    tcb = image.expression(
    '(0.3037*a + 0.2793*b + 0.4743*c + 0.5585*d + 0.5082*e + 0.1863*f)',
    {
        'a': image.select('SR_B1'), 'b': image.select('SR_B2'), 'c': image.select('SR_B3'),
        'd': image.select('SR_B4'), 'e': image.select('SR_B5'), 'f': image.select('SR_B7')
    }) \
    .rename('TCB')
    tcg = image.expression(
    '(-0.2848*a - 0.2435*b - 0.5436*c + 0.7243*d + 0.0840*e - 0.1800*f)',
    {
        'a': image.select('SR_B1'), 'b': image.select('SR_B2'), 'c': image.select('SR_B3'),
        'd': image.select('SR_B4'), 'e': image.select('SR_B5'), 'f': image.select('SR_B7')
    }) \
    .rename('TCG')
    tcw = image.expression(
    '(0.1509*a + 0.1973*b + 0.3279*c + 0.3406*d - 0.7112*e - 0.4572*f)',
    {
        'a': image.select('SR_B1'), 'b': image.select('SR_B2'), 'c': image.select('SR_B3'),
        'd': image.select('SR_B4'), 'e': image.select('SR_B5'), 'f': image.select('SR_B7')
    }) \

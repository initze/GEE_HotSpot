...
def harmonizationRoy(oli): #S harmonisierung von 8 (pixel_qa problem?, bänder?)
...
#S: qa maskierung für C2
def maskL8C2sr(image):
...
def preprocessed_L8_C1_collection(dataset_name, bbox, date_filter_yr, date_filter_mth, meta_filter_cld):
...
#S: 
def preprocessed_L8_C2_collection(dataset_name, bbox, date_filter_yr, date_filter_mth, meta_filter_cld):
  collection = ee.ImageCollection(dataset_name)\
  .filterBounds(bbox)\
  .filter(date_filter_yr)\
  .filter(date_filter_mth)\
  ...
  .map(maskLsSr)\
  .map(make_dateband)\
  .select(['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B6', 'SR_B7', 'QA_PIXEL', 'Date']) \
  .rename(['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'pixel_qa', 'Date'])  # angleichen an Collection 1
  return collection
...
def makeLandsatSeriesSr(bbox, date_filter_yr, date_filter_mth, meta_filter_cld):
  ...
  l8_1 = preprocessed_L8_C1_collection('LANDSAT/LC08/C01/T1_SR', bbox, date_filter_yr, date_filter_mth, meta_filter_cld)
  l8_2 = preprocessed_L8_C2_collection('LANDSAT/LC08/C02/T1_L2', bbox, date_filter_yr, date_filter_mth, meta_filter_cld) #S

  return l5.merge(l7).merge(l8_1).merge(l8_2)

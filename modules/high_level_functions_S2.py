...
def makeSentinelSrFiltered(config):
    # 1. load Landsat data and calculate indices
    collection = utils_LS.preprocessed_S2_collection(config['geom'], config['date_filter_yr'], config['date_filter_mth'], config['meta_filter_cld'])\
    .map(indices.ndvi) \
    .map(indices.ndmi) \
    .map(indices.ndwi) \
    .map(indices.tcS2) 
...
def runTCTrend(config_trend):
  # 1. load Landsat data and calculate indices
  collection = utils_LS.preprocessed_S2_collection(config_trend['geom'], 
